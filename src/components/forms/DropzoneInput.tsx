  ...
  return (
    <div>
      ...
      {readOnly && !(files?.length > 0) ? (
        <div className='py-3 pr-4 pl-3 text-sm rounded-md border border-gray-300 divide-y divide-gray-300'>
          No file uploaded
        </div>
      ) : files?.length >= maxFiles ? (
        <ul className='mt-1 rounded-md border border-gray-300 divide-y divide-gray-300'>
        ...
        <Controller
          ...
          render={(controllerProps) => (
            <>
              <div
                className='group mt-1 focus:ring-dark-400 focus:outline-none'
                {...getRootProps()}
                {...controllerProps}
                ref={dropzoneRef}
              >
